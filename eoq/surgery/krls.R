...
function(     X=NULL,
      ...
      print("Computing KRLS from local development version.")
      ...
      # Eigenobject <- eigen(K,symmetric=TRUE)  # Original KRLS: deterministic eigendecomposition
      
      ### Begin AK ###
      tic("Low-rank matrix approximation")
      # Replace with approximate low-rank eigendecomposition
      if(is.null(rank)) {
        rank <- nrow(K)  # Full rank SVD/eigendecompositon if not specified
        # Eigenobject <- eigen(K,symmetric=TRUE)
      } #else {
      Eigenobject <- reigen(K, k=rank)
      Eigenobject$values <- sqrt(Eigenobject$values)
      eigtrunc <- min(Eigenobject$values)
      #}
      # eigtrunc <- 0.005  # Drop zeroed-out eigenvalues
      toc()
      ...
      tic("Solve for choice coefficients")
      ...
      if(vcov==TRUE){    
        tic("Variance-covariance matrix")
        # sigma squared
        ...
        if (is.null(eigtrunc)){	
          ...
          # lastkeeper=max(which(Eigenobject$values>=eigtrunc*Eigenobject$values[1]))	
          lastkeeper <- which(Eigenobject$values == eigtrunc)  # ALEX: This is always equal to rank choice
